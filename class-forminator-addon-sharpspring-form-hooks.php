...
/**
 * Class Forminator_Addon_SharpSpring_Form_Hooks
 *
 * @since 1.0 SharpSpring Addon
 *
 */
class Forminator_Addon_SharpSpring_Form_Hooks extends Forminator_Addon_Form_Hooks_Abstract {

	/**
	 * Addon instance are auto available form abstract
	 * Its added here for development purpose,
	 * Auto-complete will resolve addon directly to `SharpSpring` instance instead of the abstract
	 ...
	 *
	 * @since 1.0 SharpSpring Addon
	 * @var Forminator_Addon_SharpSpring_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Forminator_Addon_SharpSpring_Form_Hooks constructor.
	 *
	 * @since 1.0 SharpSpring Addon
	 *
	...
	public function __construct( Forminator_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'SharpSpring failed to process submitted data. Please check your form and try again', 'forminator' );
	}

	/**
	 * Save status of request sent and received for each connected SharpSpring Connection
	 *
	 * @since 1.0 SharpSpring Addon
	 *
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		...
		/**
		 * Filter SharpSpring submitted form data to be processed
		 *
		 * @since 1.4
		 *
		 * @param array $submitted_data
		 * @param int $form_id current Form ID
		 * @param Forminator_Addon_SharpSpring_Form_Settings $form_settings_instance SharpSpring Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'forminator_addon_sharpspring_form_submitted_data',
			$submitted_data,
			$form_id,
		...
		/**
		 * Fires before send message to SharpSpring
		 *
		 * @since 1.4
		 *
		 * @param int $form_id current Form ID
		 ...
		 * @param Forminator_Addon_SharpSpring_Form_Settings $form_settings_instance SharpSpring Addon Form Settings instance
		 */
		do_action( 'forminator_addon_sharpspring_before_send_message', $form_id, $submitted_data, $form_settings_instance );

		foreach ( $addon_setting_values as $key => $addon_setting_value ) {
			// save it on entry field, with name `status-$MULTI_ID`, and value is the return result on sending data to sharpspring
		...
		$entry_fields = $data;
		/**
		 * Filter SharpSpring entry fields to be saved to entry model
		 *
		 * @since 1.4
		 *
		 ...
		 */
		$data = apply_filters(
			'forminator_addon_sharpspring_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_settings_instance
	...
	/**
	 * Get status on send message to SharpSpring
	 *
	 * @since 1.0 SharpSpring Addon
	 *
	 * @param $connection_id
	 ...
	 *
	 * @return array `is_sent` true means its success send data to SharpSpring, false otherwise
	 */
	private function get_status_on_contact_sync( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		if ( empty( $connection_settings['name'] ) ) {
			$connection_settings['name'] = __( 'SharpSpring', 'forminator' );
		}
		...
		try {
			...
			$email_element_id = $connection_settings['fields_map']['emailAddress'];
			if ( ! isset( $submitted_data[ $email_element_id ] ) || empty( $submitted_data[ $email_element_id ] ) ) {
				/* translators: ... */
				throw new Forminator_Addon_SharpSpring_Exception( __( 'Email on element %1$s not found or not filled on submitted data.', 'forminator' ) );
			}
			...
			/**
			 * Filter arguments to passed on to Contact Sync SharpSpring API
			 *
			 * @since 1.2
			 *
			 * @param array $args
			 ...
			 */
			$args = apply_filters(
				'forminator_addon_sharpspring_create_contact_args',
				$args,
				$form_id,
				$connection_id,
				$submitted_data,
			...
			return array(
				...
				'description'     => __( 'Successfully send data to SharpSpring', 'forminator' ),
			...
			forminator_addon_maybe_log( __METHOD__, 'Failed to Send to SharpSpring' );

			$addon_entry_fields = array(
				'is_sent'         => false,
				'description'     => $e->getMessage(),
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $api instanceof Forminator_Addon_SharpSpring_Wp_Api ) ? $api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $api instanceof Forminator_Addon_SharpSpring_Wp_Api ) ? $api->get_last_data_received() : array() ),
				'url_request'     => ( ( $api instanceof Forminator_Addon_SharpSpring_Wp_Api ) ? $api->get_last_url_request() : '' ),
				'contact_id'      => null
			);
	 ...
	 * subentries included are defined in @see Forminator_Addon_SharpSpring_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 SharpSpring Addon
	 *
	...
	public function on_render_entry( Forminator_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		/**
		 *
		 * Filter SharpSpring metadata that previously saved on db to be processed
		 *
		 * @since 1.4
		 *
		...
		$addon_meta_data = apply_filters(
			'forminator_addon_sharpspring_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	...
	/**
	 * Loop through addon meta data on multiple SharpSpring setup(s)
	 *
	 * @since 1.0 SharpSpring Addon
	 *
	 ...
	 *
	 * - Integration Name : its defined by user when they adding SharpSpring integration on their form
	 * - Sent To SharpSpring : will be Yes/No value, that indicates whether sending data to SharpSpring API was successful
	 * - Info : Text that are generated by addon when building and sending data to SharpSpring @see Forminator_Addon_SharpSpring_Form_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Forminator_Addon_SharpSpring::is_show_full_log() @see FORMINATOR_ADDON_SHARPSPRING_SHOW_FULL_LOG
	 *      - API URL : URL that wes requested when sending data to SharpSpring
	 *      - Data sent to SharpSpring : encoded body request that was sent
	 *      - Data received from SharpSpring : json encoded body response that was received
	 *
	 * @param $addon_meta_data
	 *
	 * @since 1.0 SharpSpring Addon
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'SharpSpring Integration', 'forminator' ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To SharpSpring', 'forminator' ),
				'value' => $is_sent,
			);
		}
		...
		if ( Forminator_Addon_SharpSpring::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('FORMINATOR_ADDON_SHARPSPRING_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to SharpSpring', 'forminator' ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from SharpSpring', 'forminator' ),
	...
	/**
	 * SharpSpring will add a column on the title/header row
	 * its called `SharpSpring Info` which can be translated on forminator lang
	 *
	 * @since 1.0 SharpSpring Addon
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'SharpSpring Info', 'forminator' ),
		);
		...
		/**
		 * Filter SharpSpring headers on export file
		 *
		 * @since 1.2
		 *
		...
		$export_headers = apply_filters(
			'forminator_addon_sharpspring_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	/**
	 * SharpSpring will add a column that give user information whether sending data to SharpSpring successfully or not
	...
	public function on_export_render_entry( Forminator_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		/**
		 * Filter SharpSpring columns to be displayed on export submissions
		 *
		 * @since 1.4
		 *
		...
		$export_columns = apply_filters(
			'forminator_addon_sharpspring_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Forminator_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		/**
		 *
		 * Filter SharpSpring addon metadata that previously saved on db to be processed
		 *
		 * @since 1.4
		 *
		 * @param array $addon_meta_data
		...
		/**
		 * Fires when SharpSpring connected form delete a submission
		 *
		 * @since 1.1
		 *
		 * @param int $form_id current Form ID
		 ...
		 */
		do_action(
			'forminator_addon_sharpspring_on_before_delete_submission',
			$form_id,
			$entry_model,
			$addon_meta_data,
			$form_settings_instance
		);
		if ( ! Forminator_Addon_SharpSpring::is_enable_delete_member() ) {
		...
		try {
			...
			$contact_to_delete = apply_filters(
				'forminator_addon_sharpspring_contact_to_delete',
				$contact_to_delete,
				$form_id,
				$addon_meta_data,
				$form_settings_instance
			...
			$wp_error
				= new WP_Error( 'forminator_addon_sharpspring_delete_contact', $e->getMessage() );
